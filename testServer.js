...
app.get('/changeRate/:from/:to', (req, res) => {
    ...
    const rate = rates.get(JSON.stringify({ from: req.params.from, to: req.params.to }))
...
app.post('/api/account', (req, res) => {
    ...
    const edit = acountCollection.find(item => item.ownerId === account.ownerId)
    if (edit) {
...
app.delete('/api/account/:id', (req, res) => {
    ...
    acountCollection = acountCollection.filter(item => item.ownerId !== ownerId);
    res.send('account added');
});
