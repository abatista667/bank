...
mock.onGet("/changeRate/USD/EUR").reply(200, {
  users: 1.1,
});
...
const mockTransactions: TransactionResponse[] = [
  { id: "1", fromOwnerId: 1, toOwnerId: 2, amount: 100, changeRate: 1.1, transferAmount: 110, transactionId: "id" },
  { id: "2", fromOwnerId: 2, toOwnerId: 1, amount: 200, changeRate: 1.1, transferAmount: 110, transactionId: "id" },
];
...
describe("TransactionList", () => {
  ...
  it("opens the transaction form when the add button is clicked", async () => {
    ...
    expect(await screen.findByRole("button", {name: "Send transaction"})).toBeInTheDocument();
  });
  ...
  it("calls addTransaction when a new transaction is saved", async () => {
    ...
    await userEvent.click(screen.getByRole("button", {name: "Create New"}));
    expect(await screen.findByRole("button", {name: "Send transaction"})).toBeDisabled()
    ...
    expect(await screen.findByRole("button", {name: "Send transaction"})).not.toBeDisabled()
    await userEvent.click(await screen.findByRole("button", {name: "Send transaction"}));
